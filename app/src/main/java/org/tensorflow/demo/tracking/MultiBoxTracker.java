...
public class MultiBoxTracker {
  // TODO Martin crear clases graba log y senddata
  //martin
  private static final GrabaLog GRABA_LOG = new GrabaLog();
  private static final senddata SENDDATA = new senddata();
  private final ArrayList<Integer> objetox = new ArrayList<Integer>();
  public static Map<Integer,Instant> mymap = new HashMap<Integer, Instant>();
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      //martin registra mediante GrabaLog el objeto detectado a un archivo con ubicación GPS del mobile y fecha hora.
      SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd-HH:mm:ss:SSS");
      String horaTaller = sdf1.format(new Date());
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
      String currentDateandTime = sdf.format(new Date());
      String objeto = recognition.title;
      String hoydia = Contadores.hoy;
      Float porcentaje = recognition.detectionConfidence;
      String strAmount=String.valueOf(porcentaje);
      String colores = String.valueOf(recognition.color);
      //Compara si el color trackeado es igual al de algun otro en el frame.
      // Almacena hasta 30 colores o id y luego los va borrando
      if(objetox.contains(recognition.color)){
        //logger.w("Objetox contiene al color");
      }
      else {
        if( Integer.parseInt(hoydia.substring(9,10)) != Integer.parseInt(currentDateandTime.substring(9,10))){
          Contadores.CuentaObjetos.clear();
          Contadores.CuentaObjetosFireB.clear();
          Contadores.ListaObjetos.clear();

          logger.i("Cambio de día: Ayer " + hoydia.substring(9,10)+", hoy: "+currentDateandTime+" - "+ new Date());
          Contadores.hoy = currentDateandTime;
          GRABA_LOG.SetLog("hoy:"+currentDateandTime);
          logger.i("seteando nuevo día de hoy:"+Contadores.hoy);
        }else{}
          objetox.add(recognition.color);
          mymap.put(recognition.color, Instant.now());
          //GRABA_LOG.appendLog("fecha:"+currentDateandTime + " lat_log:" + latitude + "_" +longitude+ " objeto:"+ objeto + " %:" + strAmount );
          GRABA_LOG.appendLog("fecha:"+currentDateandTime + " lat_log:" + "_" + " objeto:"+ objeto + " %:" + strAmount );
                  //TODO Martin Crear Firefox Senddata
         SENDDATA.CarDetect("lat:"+ Contadores.lat +"@lon:"+Contadores.lon, currentDateandTime , objeto, 1);
         //log para Taller
        logger.i("taller obj in: "+ objeto +" id: "+ colores+" H:"+horaTaller);
        String currentDateandTimelast = currentDateandTime;
  ...
  public synchronized void onFrame(
    ...
    for (final TrackedRecognition recognition : copyList) {
      ...
      if (correlation < MIN_CORRELATION) {
        ...
        //TODO martin quita los colores que ya no se usan para seguir contando
        if(objetox.contains(recognition.color)){
          //log para Taller
          ...
          SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd-HH:mm:ss:SSS");
          String horaTaller = sdf1.format(new Date());
          Instant horaIn = mymap.get(recognition.color);
          Instant horaOut = Instant.now();
          long ns = Duration.between(horaIn, horaOut).toMillis();
          String nss = String.valueOf(ns/1000);
          String strhin = String.valueOf(horaIn);
          String strhout = String.valueOf(horaOut);
          logger.i("taller #11; obj id: "+recognition.color+" tipo: "+recognition.title+" in:"+strhin+" out: "+strhout + " Dif: " + nss+ " seg.");

                    //    logger.i("taller #11; obj out: "+recognition.title+" id: "+recognition.color+" H:"+horaTaller+" in: "+ nss +" ,objetox size"+objetox.size()+" objXpos "+objetox.indexOf(recognition.color)+" ");
          int objx = objetox.indexOf(recognition.color);
          objetox.remove(objx);
          // objetox.remove(recognition.color);
            mymap.remove(recognition.color);
